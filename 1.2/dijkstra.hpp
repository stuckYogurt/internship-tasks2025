...
void Dijkstra_path(Graph& graph, Node* root_node) {
    ...
    auto distances = new std::vector<EDGE_WEIGHT_TYPE>(node_count, std::numeric_limits<EDGE_WEIGHT_TYPE>::max());
    ...
    for (uns _ = 0; _ < node_count; _++) {
        ...
        for (auto Edge : Edges)
            (*distances)[Edge->getDrain()->getId()] = std::min((*distances)[Edge->getDrain()->getId()],
                                                               (*distances)[v] + Edge->getWeight());
    ...
    for (uns i = 0; i < node_count; i++) {
        ...
        ((*distances)[i] == std::numeric_limits<EDGE_WEIGHT_TYPE>::max() ? "inf" : std::to_string((*distances)[i])) <<
        std::endl;
    }
